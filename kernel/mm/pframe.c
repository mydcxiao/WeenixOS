...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        *result = pframe_get_resident(o, pagenum);
        ...
        if (*result == NULL) { // If the page is not resident, allocate a new page
                // Check if we need to call pageoutd
                //if (pageoutd_needed()) {
                //        pageoutd_wakeup();
                //        dbg(DBG_PRINT, "(GRADING3D)\n");
                //}
                ...
                *result = pframe_alloc(o, pagenum);
                ...
                // Fill the page
                if (pframe_fill(*result) < 0) {
                        pframe_free(*result);
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        return -EFAULT;
                }
                dbg(DBG_PRINT, "(GRADING3D)\n");
        } else { // the page is found
                // If the page is busy, wait for it to become unbusy
...
pframe_pin(pframe_t *pf)
{
        ...
        if (pf->pf_pincount == 0) { // If the page has not been pinned
                // remove list link from the list
                list_remove(&pf->pf_link);
