...
do_brk(void *addr, void **ret)
{
        ...
        uint32_t old_vma_end = vma->vma_end;
        uint32_t new_vma_end = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
        if(new_vma_end > old_vma_end)
        {
                if(!vmmap_is_range_empty(curproc->p_vmmap, old_vma_end, new_vma_end - old_vma_end))
                {
                        dbg(DBG_PRINT, "(GRADING3D 2)\n");
                        return -ENOMEM;
                }
                vma->vma_end = new_vma_end;
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
        else if(new_vma_end < old_vma_end)
        {
                vmmap_remove(curproc->p_vmmap, new_vma_end, old_vma_end - new_vma_end);
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
        }
        curproc->p_brk = addr;
        *ret = addr;
