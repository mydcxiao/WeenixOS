...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        //if ((!(vma->vma_prot & PROT_READ) && (perm & PROT_READ)) || 
        //        (!(vma->vma_prot & PROT_WRITE) && (perm & PROT_WRITE)) ||
        //        (!(vma->vma_prot & PROT_EXEC) && (perm & PROT_EXEC))) { 
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t start = (uint32_t)avaddr;
        uint32_t end = (uint32_t)avaddr + len;
        dbg(DBG_PRINT, "(GRADING3D)\n");

        // Check every page start point in the range
        uint32_t i = start;
        while (i <= end) {
                if (!addr_perm(p, (void *)i, perm)) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        return 0;
                }
                i += PAGE_SIZE;
