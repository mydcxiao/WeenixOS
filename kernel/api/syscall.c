...
sys_read(read_args_t *arg)
{
        ...
        //if ((err = copy_from_user(&kernel_args, arg, sizeof(read_args_t))) < 0) {
        //        curthr->kt_errno = -err;
        ...
        // call do_read(), and copy_to_user() the read bytes
        int total_bytes = 0;
        while (total_bytes < ((int)kernel_args.nbytes)) {
                // each time, read at most PAGE_SIZE bytes
                int bytes_target = ((uint32_t)kernel_args.nbytes - total_bytes > PAGE_SIZE) ? PAGE_SIZE : ((uint32_t)kernel_args.nbytes - total_bytes);
                int bytes_read = do_read(kernel_args.fd, buffer, bytes_target);
                if (bytes_read < 0) {
                        page_free(buffer);
                ...
                copy_to_user((void *)((uint32_t)kernel_args.buf + total_bytes), buffer, bytes_read);
                total_bytes += bytes_read;
                if (bytes_read < bytes_target) { // cannot read more bytes
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        break;
                }
                dbg(DBG_PRINT, "(GRADING3D)\n");
...
sys_write(write_args_t *arg)
{
        ...
        //if ((err = copy_from_user(&kernel_args, arg, sizeof(write_args_t))) < 0) {
        //        curthr->kt_errno = -err;
        //        dbg(DBG_PRINT, "(GRADING3D)\n");
        //        return -1;
        //}
        ...
        while (total_bytes < ((int)kernel_args.nbytes)) {
                // each time, read at most PAGE_SIZE bytes
                int bytes_target = ((uint32_t)kernel_args.nbytes - total_bytes > PAGE_SIZE) ? PAGE_SIZE : ((uint32_t)kernel_args.nbytes - total_bytes);
                copy_from_user(buffer, (void *)((uint32_t)kernel_args.buf + total_bytes), bytes_target);
                //if ((err = copy_from_user(buffer, (void *)((uint32_t)kernel_args.buf + total_bytes), bytes_target)) < 0) {
                //        page_free(buffer);
                //        curthr->kt_errno = -err;
                //        dbg(DBG_PRINT, "(GRADING3D)\n");
                //        return -1;
                //}
                int bytes_write = do_write(kernel_args.fd, buffer, bytes_target);
                if (bytes_write < 0) {
                        page_free(buffer);
                ...
                dbg(DBG_PRINT, "(GRADING3D)\n");
                //if (bytes_write < bytes_target) { // cannot write more bytes
                //        dbg(DBG_PRINT, "(GRADING3D)\n");
                //        break;
                //}
                dbg(DBG_PRINT, "(GRADING3D)\n");
...
sys_getdents(getdents_args_t *arg)
{
        ...
        //if ((err = copy_from_user(&kernel_args, arg, sizeof(getdents_args_t))) < 0) {
        //        curthr->kt_errno = -err;
        //        dbg(DBG_PRINT, "(GRADING3D)\n");
        //        return -1;
        //}
        ...
        // allocate a dirent_t
        dirent_t dirent;
        int loop_times = kernel_args.count / sizeof(dirent_t);
        dbg(DBG_PRINT, "(GRADING3D)\n");

        // call do_getdent in a loop until you have read getdent_args_t->count bytes
        int total_bytes = 0;
        int i;
        for (i = 0; i < loop_times; i++) {
                int bytes_read = do_getdent(kernel_args.fd, &dirent);
                if (bytes_read < 0) {
                        curthr->kt_errno = -bytes_read;
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        return -1;
                ...
                copy_to_user(kernel_args.dirp + i, &dirent, bytes_read);
                total_bytes += bytes_read;
                dbg(DBG_PRINT, "(GRADING3D)\n");
        }
        dbg(DBG_PRINT, "(GRADING3D)\n");
